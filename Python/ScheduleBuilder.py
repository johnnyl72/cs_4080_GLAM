    ...
    for employee in reversed(ListofEmployees):
        for week in employee.schedule:
            ...
            for k,v in temp.items():
                if v == 'WORKING':
                    schedule[Week][k] = employee.name
                elif v == 'OFF' and schedule[Week][k] == employee.name:
                    schedule[Week][k] = 'Empty'
            Week += 1
        Week = 0
    return schedule
    ...
    for employee in ListofEmployees:
        for i in employee.schedule:
            ...
            for key,value in temp.items():
                for j in range(2):
                    if schedule[j][key] == 'Empty' and value == 1:
                        schedule[0][key] = employee.name
                        schedule[1][key] = employee.name
                        employee.schedule[0][key] = 1
                        employee.schedule[1][key] = 1

                    elif schedule[j][key] != employee.name and value == 1:
...
if __name__ == "__main__":
    ...
    print("\n")
    print("Approved Steven's Swap Request")
